...
namespace Tugas_PBO_TM__Ref_dan_Out_
{
    class Program
    {
    ...
    class getData
    {
        ...
        {
            return new NpgsqlConnection(@"server=localhost;port=5432;user id=postgres;password=w*cEd6aL;database=dbtugasrefout;");
        }
    ...
    class crud
    {
        ...
        public bool insert(ref bool info)
        {
            try
            {
                NpgsqlConnection con = dbconnection();
                con.Open();
                NpgsqlCommand cmd = new NpgsqlCommand("insert into buku(id_buku,nama,jumlah_halaman) values(3,'Buku Sakti Hengker', 289)", con);
                cmd.ExecuteNonQuery();
                cmd.Dispose();
                con.Close();
                info = true;
        ...
        public bool update(ref bool info)
        {
            try
            {
                NpgsqlConnection con = dbconnection();
                con.Open();
                NpgsqlCommand cmd = new NpgsqlCommand("update buku set nama = Cara Menahan Puasa Seperti MU, jumlah_halaman = 90 where id_buku = 3", con);
                cmd.ExecuteNonQuery();
                cmd.Dispose();
                con.Close();
                info = true;
